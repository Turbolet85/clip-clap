...
- 2026-04-18: `go test -race` is unavailable on clip-clap — the race detector requires CGO_ENABLED=1 (`go: -race requires cgo`), but architecture §[CGO Policy] mandates CGO_ENABLED=0 workflow-wide. Drop `-race` from all local + CI test invocations; rely on (a) `sync.Mutex`/`sync.RWMutex` in mutable-state code (e.g., `internal/status.versionMu` around `injectedVersion`, `internal/lasterror` atomic.Value wrapper), (b) careful code review of goroutine boundaries (status server, message pump, overlay pump, hotkey pump), and (c) integration-test surfacing of races under actual load. **Why:** this cascades from the CGO=0 policy (cross-compilation, Defender false-positives, no MinGW) which is non-negotiable. **How to apply:** when writing new goroutine-touching code, over-document the synchronization contract in comments (as `internal/status/handler.go::SetVersion` does) since automated detection is off the table.
